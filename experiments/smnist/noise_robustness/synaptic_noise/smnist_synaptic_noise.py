...
def add_noise(model, noise_p: float):
    for param in model.parameters():
        if len(param.shape) == 2:
            # calculate standard deviation proportional to the weight
            std_dev = noise_p * param.data
            param.data += std_dev*torch.randn_like(param)
    return model

########################################################################################
# Test noise level on weights
########################################################################################

noise_std = np.arange(0, 1.1, 0.1)
print(noise_std)

models_str, comment = saved_models(neuron_=neuron)
repeats = 5
losses = np.zeros((len(noise_std), len(models_str), repeats))
accs = np.zeros((len(noise_std), len(models_str), repeats))
sop = np.zeros((len(noise_std), len(models_str), repeats))
...
for j in range(len(models_str)):
    ...
    for i in range(len(noise_std)):
        ...
        for h in range(repeats):
            ...
            model = add_noise(model, noise_std[i])
...
# np.save('./smnist_noise_weights_5reps_{}'.format(neuron), (losses, accs, sop))
